...
def mock_base_aio_config():
    """Create a mock base AIOConfig for AppContext."""
    return AIOConfig(
...
def _build_test_mcp(aio_config: AIOConfig | None, read_only: bool) -> AIOTestsMCP:
    ...
    async def test_lifespan(app: FastMCP) -> AsyncGenerator[dict, None]:
        ...
        try:
            yield {
                "app_lifespan_context": AppContext(
...
def mock_request():
    ...
    request.state.aio_service_headers = {}
    return request
