"""SSL-related utility functions for the AIO Tests MCP server."""
...
logger = logging.getLogger("aio-tests-mcp")


class SSLIgnoreAdapter(HTTPAdapter):
    """HTTP adapter that ignores SSL verification.

    Disables certificate verification for specific domains by setting both
    verify_mode to CERT_NONE and check_hostname to False, which is required
    to properly ignore SSL certificates.

    It also enables legacy SSL renegotiation, which some older servers need.
    This reduces security and should only be used when absolutely necessary.
    """
...
def configure_ssl_verification(
    ...
    Args:
        service_name: Name of the service for logging (e.g., "AIO Tests")
